...
- 遵循 Artifact 生成类边界：可以写入对应 `.spec-flow-kit/` 文件，不主动修改业务源码，可运行必要的只读或低风险命令。
- 不切换 Git branch。
- 不要在最终输出中重复声明权限边界，除非用户明确询问、本次操作因为权限边界被跳过，或需要解释为什么没有执行某个动作。
...
- 遵循开发类边界：可以在目标 feature 和 task 范围内修改业务代码、测试代码并运行必要验证。
- 不执行 install、deploy、release、production、数据迁移或破坏性命令，除非用户明确确认。
- 不设置 `verification-passed`；最终验收由 `/sfk-verify` 负责。
- 不要在最终输出中重复声明权限边界，除非用户明确询问、本次操作因为权限边界被跳过，或需要解释为什么没有执行某个动作。
...
- 遵循验证类边界：可以读取源码、测试、配置并运行 lint、typecheck、test、build。
- 默认不做大范围实现改动；如需小修复，必须与验证结果分开说明，并重新运行相关验证。
- 不执行 install、deploy、release、production、数据迁移或破坏性命令，除非用户明确确认。
...
- 遵循只读/诊断类边界：默认只读，不主动修改源码或 `.spec-flow-kit/` artifact，不主动运行 Bash。
- 如需运行命令，应说明目的，并在安全命令白名单或用户确认范围内执行。
- 不要在最终输出中重复声明权限边界，除非用户明确询问、本次操作因为权限边界被跳过，或需要解释为什么没有执行某个动作。
...
- 只读/诊断类（status、next、audit）：默认只读，不主动修改源码或 `.spec-flow-kit/` artifact，不主动运行 Bash；如需运行命令，应说明目的，并在安全命令白名单或用户确认范围内执行。
- Artifact 生成类（init、requirements、design、plan、rules-sync、use）：可以写入对应 `.spec-flow-kit/` 文件；不主动修改业务源码，除非用户明确要求且范围很小；可以运行必要的只读或低风险命令；不默认运行 install、test、build、deploy。
- 验证类（verify）：可以读取源码、测试、配置并运行 lint、typecheck、test、build；默认不做大范围实现改动；危险命令需要明确确认。
- 开发类（development）：可以在目标 feature 和 task 范围内修改业务代码、测试代码并运行必要验证；不设置 `verification-passed`。
- 交付/部署准备类（deliver、deploy）：默认只生成交付说明、风险说明、runbook、环境检查和回滚步骤；不真正执行部署、发布、生产变更或数据迁移。
- 所有命令：不要在最终输出中重复声明权限边界，除非用户明确询问、本次操作因为权限边界被跳过，或需要解释为什么没有执行某个动作。
